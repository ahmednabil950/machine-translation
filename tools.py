...
def to_dict(data_df, columns=[], serialize=True):
    data_dict = None
    if not len(columns):
        data_dict = data_df.to_dict()
    else:
        data_dict = data_df[columns].to_dict()
    return data_dict if not serialize else json.dumps(data_dict)

def filter_by_column(df, column, value):
    filtered = df.query('%s == @value' % (column,))
    return filtered
...
def strip_punctuations(qry):
    qry = str(qry)
    translator = str.maketrans('', '', string.punctuation)
    return qry.translate(translator)

def strip_stopwords(qry):
    qry = str(qry)
    return " ".join(w for w in qry.split() if w not in STOPWORDS)

def strip_extra_whitespaces(qry):
    RE_WHITESPACE = re.compile(r"(\s)+", re.UNICODE)
    qry = str(qry)
    return RE_WHITESPACE.sub(" ", qry)

def strip_numeric(qry):
